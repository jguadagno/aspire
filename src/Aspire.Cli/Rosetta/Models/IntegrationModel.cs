...
internal class IntegrationModel
{
    ...
    /// </code>
    /// </example>
    public List<RoMethodInfo> IDistributedApplicationBuilderExtensionMethods { get; } = [];

    public List<RoMethodInfo> SharedExtensionMethods { get; } = [];
    ...
    internal IEnumerable<RoMethodInfo> GetExtensionMethods(IWellKnownTypes wellKnownTypes, RoType extendedType)
    {
        return GetExtensionMethods(Assembly, wellKnownTypes, extendedType);
    }

    private static IEnumerable<RoMethodInfo> GetExtensionMethods(RoAssembly assembly, IWellKnownTypes wellKnownTypes, RoType extendedType)
    {
        ...
        bool HasAttribute(RoMethodInfo method, RoType attributeType)
        {
            return method.GetCustomAttributes().Any(attr => attr.AttributeType == attributeType);
        }
    }

    public void DiscoverModelClasses(List<RoMethodInfo> methods, HashSet<RoType> modelTypes)
    {
        ...
        void ScanMethod(RoMethodInfo method)
        {
