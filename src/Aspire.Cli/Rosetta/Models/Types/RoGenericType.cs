...
internal sealed class RoGenericType : RoType
{
    private readonly RoType _genericTypeDefinition;
    private readonly IReadOnlyList<RoType> _genericArguments;

    public RoGenericType(RoType genericTypeDefinition, IReadOnlyList<RoType> genericArguments)
    ...
    public override IReadOnlyList<RoType> GenericArguments => _genericArguments;
    public override IReadOnlyList<RoType> GenericTypeArguments => _genericArguments;
    public override bool ContainsGenericParameters => _genericArguments.Any(a => a.ContainsGenericParameters);

    public override RoType MakeGenericType(params RoType[] typeArguments)
    {
        return new RoGenericType(_genericTypeDefinition, typeArguments);
    }
    ...
    private string BuildName(bool includeNamespace)
    {
        ...
        var args = string.Join(", ", _genericArguments.Select(a => includeNamespace ? a.FullName : a.Name));
