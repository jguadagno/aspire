...
internal class ResourceModel
{
    ...
    /// </code>
    /// </example>
    public List<RoMethodInfo> IResourceTypeBuilderExtensionsMethods { get; } = [];
    ...
    public void DiscoverExtensionMethods(IntegrationModel integrationModel)
    {
        ...
        var openGenericMethods = new List<RoMethodInfo>();

        foreach (var m in integrationModel.SharedExtensionMethods)
        {
            var genArgs = m.GenericArguments;
            ...
            if (genArg.GenericParameterConstraints.All(x => allInterfacesForThisResource.Contains(x) || allTypeForThisResource.Contains(x)))
            {
