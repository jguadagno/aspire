    ...
    public List<IntegrationModel> ResolveIntegrations(IDependencyContext dependencyContext, AssemblyLoaderContext assemblyLoaderContext, bool debug)
    {
        ...
        while (queue.TryDequeue(out var package))
        {
            ...
            foreach (var assemblyFullPath in dependencyContext.GetAssemblyPaths(name, version))
            {
                ...
                // Load all dependent assemblies with the AssemblyLoaderContext such that creating the integration will find all the required types.
                foreach (var assembly in assemblies)
                {
                    ...
                    {
                        if (!assemblyLoaderContext.LoadedAssemblies.ContainsKey(dependency))
                        {
                            assemblyLoaderContext.LoadAssembly(Path.Combine(dependencyContext.ArtifactsPath, $"{dependency}.dll"));
                        }
                    }
                }
            }
            ...
            var systemRuntimePath = Path.Combine(dependencyContext.ArtifactsPath, "System.Private.CoreLib.dll");
            var systemRuntime = assemblyLoaderContext.LoadAssembly(systemRuntimePath);

            if (systemRuntime is null)
            {
                throw new InvalidOperationException("System.Private.CoreLib.dll assembly not found.");
            }
            var wellKnownAssemblies = assemblies
                .Concat([aspireHosting, systemRuntime])
                .Where(a => a is not null)!
                .Distinct();

            var wellKnownTypes = new WellKnownTypes(wellKnownAssemblies);
