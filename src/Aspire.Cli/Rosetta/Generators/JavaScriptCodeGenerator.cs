...
internal sealed class JavaScriptCodeGenerator(ApplicationModel appModel, IInteractionService interactionService) : ICodeGenerator
{
    ...
    private readonly Dictionary<RoMethodInfo, string> _overloadParameterClassByMethod = [];
    ...
    private void GenerateMethods(TextWriter writer, IEnumerable<RoMethodInfo> extensionMethods, string ctorArgs)
    {
    ...
    private string FormatArgument(RoParameterInfo p)
    {
        ...
        if (IsNullableOfT)
        {
            result += $": {FormatJsType(p.ParameterType.GenericArguments[0])} | null";
        }
        else
        {
    ...
    private string FormatJsType(RoType type)
    {
        return type switch
        {
            ...
            { IsGenericType: true } when type.GenericTypeDefinition == _appModel.WellKnownTypes.GetKnownType(typeof(Nullable<>)) => FormatJsType(type.GenericArguments[0]) + " | null",
