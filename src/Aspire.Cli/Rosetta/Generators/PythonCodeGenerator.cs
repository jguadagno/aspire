...
internal sealed class PythonCodeGenerator(ApplicationModel appModel) : ICodeGenerator
{
    ...
    private void GenerateResourceExtensionMethods(TextWriter writer, RoType type, IEnumerable<RoMethodInfo> extensionMethods)
    {
        foreach (var methodGroups in extensionMethods.GroupBy(m => m.Name))
        {
            ...
            foreach (var overload in overloads)
            {
                ...
                if (returnType.IsGenericType && returnType.GenericTypeDefinition == _appModel.WellKnownTypes.IResourceBuilderType)
                {
                    returnType = returnType.ContainsGenericParameters ? type : returnType.GenericArguments[0];
                    pyReturnTypeName = returnType.Name + "Builder";
                }
    ...
    private string FormatArgument(RoParameterInfo p)
    {
        ...
        {
            result += $": Optional[{FormatPyType(p.ParameterType.GenericArguments[0])}] = None";
        }
        else
        {
            if (p.IsOptional)
    ...
    private string FormatCsArgument(RoParameterInfo p, string prefix)
    {
        ...
        if (p.ParameterType.IsGenericType &&
            ...
            p.ParameterType.GenericArguments[0] is { } genericArgument &&
            ...
            genericArgument.GenericTypeDefinition == _appModel.WellKnownTypes.IResourceBuilderType)
        {
            ...
            var resourceType = genericArgument.GenericArguments[0];
    ...
    private string FormatPyType(RoType type)
    {
        return type switch
        {
            ...
            { IsGenericType: true } when type.GenericTypeDefinition == _appModel.WellKnownTypes.IResourceBuilderType => $"{type.GenericArguments[0].Name}Builder",
            { IsGenericType: true } when type.GenericTypeDefinition == _appModel.WellKnownTypes.GetKnownType(typeof(Nullable<>)) => $"Optional[{FormatPyType(type.GenericArguments[0])}]",
    ...
    private static string FormatMethodSignature(RoMethodInfo method)
    {
    ...
    private static string PrettyPrintCSharpType(RoType? t)
    {
        ...
        if (t.IsGenericType)
        {
            var genericType = t.GenericTypeDefinition;
            var genericArguments = t.GenericArguments;
            var genericArgumentList = string.Join(", ", genericArguments.Select(PrettyPrintCSharpType));
